...
const LiveChat = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      const res = await fetch(
        `https://property-hunter-server-roan.vercel.app/api/v1/users/${user?.id}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  // call AudioVideoCall component
  const handleVideoCall = () => {
    // Open the video call component in a new tab
    window.open('/dashboard/audioVideoCall', '_blank');
  };
  ...
  return (
    <div className="w-full flex bg-white dark:bg-primary-dark overflow-x-hidden">
      ...
      <div className="w-[22%] border-2 dark:bg-primary-dark dark:border-primary-dark border-white h-screen  overflow-y-scroll">
        ...
        {/* conversations div */}
        <div className="ml-5 font-bold text-black text-2xl dark:text-[#8f9396] ">
          Messages
        </div>
        <div>
          {conversations.length > 0 ? (
            conversations.map(({ conversationId, user }, i) => {
              return (
                ...
                <div
                  key={i}
                  className="hover:bg-gray-300 dark:hover:bg-[#313233]"
                >
                  <div
                    className="flex cursor-pointer gap-4 ml-5 text-sm items-center mt-2 py-4 "
                  ...
                  >
                    ...
                    <div className=" font-medium mr-8">
                      <h3 className="dark:text-[#f0f8ff] text-black">
                        {user?.name}
                      </h3>
                      <p className="dark:text-[#8f9396] text-gray-500">
                        {user?.email}
                      </p>
                    </div>
                  </div>
            ...
            <div className="text-center text-lg font-semibold text-black mt-24 dark:text-[#8f9396]">
              {' '}
              No Conversation
            </div>
          )}
          ...
          <div className="h-[800px] px-10 py-14 text-black text-xs">
            {messages?.messages?.length > 0 ? (
              messages.messages.map(({ message, user: { id } = {} }) => {
                return (
                  <div
                    className={` max-w-[40%] mb-6 rounded-b-xl p-4 ${
                      id === user?.id
                        ? 'bg-[#0084ff] text-[#e4e6cd] rounded-tl-xl ml-auto'
                        : 'dark:bg-[#303030] text-black bg-[#f0f0f0] dark:text-[#e4e6cd] rounded-tr-xl'
                    }    dark:bg-[#242526] rounded-b-xl rounded-tr-xl p-2`}
                  >
                    {message}
                  </div>
                );
              ...
              <div className=" bg-gray-600 ml-auto rounded-b-xl rounded-xl p-2 text-center">
                {' '}
                No message
              </div>
            )}
        ...
        {messages?.receiver?.fullName && (
          <div className="w-full flex justify-between dark:bg-[#242526] items-center shadow-2xl py-5 ">
            ...
            <div className="cursor-pointer p-2">
              <img
                className="rounded-box border-2 border-black cursor pointer"
                src={plus}
                alt=""
              />
            </div>
